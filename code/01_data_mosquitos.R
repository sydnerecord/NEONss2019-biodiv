...
# observations = field + sorting + expertTaxonomist + archive data combined. 
#    Includes taxonID and namedLocation for joining to other tables
# taxon = taxononic info for species contained in observations table
# location = location info for records in observations table

# Reconstruction is then: location -> (left_join) -> observations -> (left_join) -> taxon

# location
table_location <- mos_dat %>%
  select(namedLocation, decimalLatitude, decimalLongitude) %>%
  distinct() %>%
  rename(
