...
tick_location_ancillary <- tick_site_env %>%
  ...
  select(
    ...
    variable_name,
    value)
length(unique(tick_location_ancillary$location_id)) # 284
...
# lots of 0s 
nrow(tick_observation_table) # 190k records
...
tick_site_env %>%
  select(uid_field, targetTaxaPresent, totalSampledArea, samplingMethod, samplingProtocolVersion, remarks_field) %>%
  mutate(observation_ancillary_id = row_number(), totalSampledArea = as.character(totalSampledArea)) %>%
  pivot_longer(targetTaxaPresent:remarks_field, names_to = "variable_name", values_to = "value") %>%
